...
const postLuuHuyLuuAnh = async (req, res) => {
  try {
    ...
    if (data) {
      ...
      const newdata = await models.luu_anh.create({
        nguoi_dung_id,
        hinh_id,
        ngay_luu,
      });
...
const postHinhAnh = async (req, res) => {
  try {
    ...
    if (
      ...
      if (size < 2000000) {
        const data = await models.hinh_anh.create({
          ten_hinh,
          duong_dan: `/public/images/${filename}`,
          mo_ta,
          nguoi_dung_id,
        });
        data.duong_dan = `${config.DOMAIN}${data.duong_dan}`;
